...
class Node:
    def __init__(self, env, node_id, graph):
        self.env = env
...
save_graph_to_file(graph, 'network.txt')

class NetworkSimulator:
    
    def __init__(self, seed, graph_filename=None):
        self.seed = seed  
    ...
    def generate_traffic(self):
        ...
        num_pairs = 20
        self.nodes = {node_id: Node(self.env, node_id, self.graph) for node_id in self.graph.nodes()}
    ...
    def gather_statistics(self):
        ...
        print(f"Total successful transmissions: {self.successful_transmissions}")
        print(f"Total failed transmissions: {self.failed_transmissions}")
        
        if self.packet_count > 0:
            success_rate = (self.successful_transmissions / self.packet_count) * 100
            print(f"Success rate: {success_rate:.2f}%")
            print(f"Average transmission time for successful transmissions: {average_packet_transmission_time:.2f} time units")
        else:
            print("No packets were generated.")
